...
export const metadata: Metadata = {
  title: "Biaya Maintenance | Sistem Sewa Alat Berat",
};

export const dynamic = "force-dynamic";

export default async function BiayaMaintenancePage() {
  await requirePageAccess("biaya_maintenance");
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white/90">Biaya Maintenance</h2>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Ringkasan biaya dan work order maintenance per unit alat berat.
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <ResponsiveDataView
          ...
          emptyMessage="Tidak ada data biaya maintenance."
          columns={[
            {
              header: "Unit",
              render: (row) => (
            ...
            {
              header: "WO Aktif",
              render: (row) => row.woAktif > 0 ? <Badge size="sm" color="warning">{row.woAktif}</Badge> : <span className="text-sm text-gray-400">0</span>,
            },
