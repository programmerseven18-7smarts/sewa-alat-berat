...
export default function SparepartClient() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/master/sparepart?search=${encodeURIComponent(search)}`);
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white/90">Master Spare Part</h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Kelola data spare part dan stok gudang</p>
        </div>
      ...
      <div className="relative max-w-sm">
        ...
        <input
          type="text" placeholder="Cari nama atau kode spare part..."
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="max-w-full overflow-x-auto">
          <Table>
            <TableHeader className="border-gray-100 dark:border-gray-800 border-y">
              <TableRow>
                {["Kode", "Nama Spare Part", "Satuan", "Harga Satuan", "Stok", "Supplier", "Aksi"].map((h) => (
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
              {loading ? (
                ...
                <TableRow><TableCell className="py-8 text-center text-gray-400" colSpan={7}>Tidak ada data</TableCell></TableRow>
              ) : data.map((sp) => (
                <TableRow key={sp.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                  <TableCell className="py-3 px-4 font-mono text-xs font-semibold text-brand-600 dark:text-brand-400">{sp.kode}</TableCell>
                  <TableCell className="py-3 px-4 font-medium text-gray-800 text-theme-sm dark:text-white/90">{sp.nama}</TableCell>
                  <TableCell className="py-3 px-4 text-gray-500 text-theme-sm dark:text-gray-400">{sp.satuan || "-"}</TableCell>
                  <TableCell className="py-3 px-4 text-gray-800 text-theme-sm dark:text-white/90 font-medium">{formatRupiah(Number(sp.harga_satuan))}</TableCell>
                  <TableCell className="py-3 px-4">
                    <Badge size="sm" color={Number(sp.stok) > 10 ? "success" : Number(sp.stok) > 0 ? "warning" : "error"}>
                      {sp.stok} unit
                    </Badge>
                  </TableCell>
                  <TableCell className="py-3 px-4 text-gray-500 text-theme-sm dark:text-gray-400">{sp.supplier_nama || "-"}</TableCell>
                  <TableCell className="py-3 px-4">
        ...
        <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-800">
          <p className="text-sm text-gray-400">{data.length} spare part ditemukan</p>
        </div>
      </div>
    </div>
  );
