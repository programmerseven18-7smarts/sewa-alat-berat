...
const STATUS_OPTIONS = ["", "Pending", "Diproses", "Ditolak", "Deal"];
...
export default function PermintaanClient() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    const res = await fetch(`/api/sewa/permintaan?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}`);
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 dark:text-white/90">Permintaan Sewa</h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar permintaan penyewaan alat berat dari customer</p>
        </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Cari no. permintaan atau customer..." value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="max-w-full overflow-x-auto">
          <Table>
            <TableHeader className="border-gray-100 dark:border-gray-800 border-y">
              <TableRow>
                {["No. Permintaan", "Customer", "Tgl Permintaan", "Jenis Alat", "Lokasi", "Mulai Sewa", "Akhir Sewa", "Est. Jam", "Status", "Aksi"].map((h) => (
                ...
                <TableRow>
                  <TableCell className="py-12 text-center" colSpan={10}>
                    <div className="flex flex-col items-center gap-2 text-gray-400">
                      <svg viewBox="0 0 24 24" fill="currentColor" className="size-10 opacity-30"><path d="M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.89 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z"/></svg>
                      <p className="text-sm">Belum ada permintaan sewa</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : data.map((p) => (
                <TableRow key={p.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                  <TableCell className="py-3 px-4 font-semibold text-brand-600 text-theme-sm dark:text-brand-400 whitespace-nowrap">{p.no_permintaan}</TableCell>
                  ...
                  <TableCell className="py-3 px-4 text-gray-500 text-theme-sm dark:text-gray-400">{p.jenis_alat || "-"}</TableCell>
                  <TableCell className="py-3 px-4 text-gray-500 text-theme-sm dark:text-gray-400 max-w-[150px] truncate">{p.lokasi || "-"}</TableCell>
                  <TableCell className="py-3 px-4 text-gray-500 text-theme-sm dark:text-gray-400 whitespace-nowrap">{p.mulai_sewa ? formatDate(p.mulai_sewa) : "-"}</TableCell>
                  <TableCell className="py-3 px-4 text-gray-500 text-theme-sm dark:text-gray-400 whitespace-nowrap">{p.akhir_sewa ? formatDate(p.akhir_sewa) : "-"}</TableCell>
                  <TableCell className="py-3 px-4 text-gray-500 text-theme-sm dark:text-gray-400">{p.estimasi_jam ? `${p.estimasi_jam} jam` : "-"}</TableCell>
                  <TableCell className="py-3 px-4"><Badge size="sm" color={statusColor(p.status)}>{p.status}</Badge></TableCell>
                  <TableCell className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <button className="rounded p-1 text-gray-400 hover:text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-500/10" title="Proses">
        ...
        <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-800">
          <p className="text-sm text-gray-400">{data.length} permintaan ditemukan</p>
        </div>
      </div>
    </div>
  );
